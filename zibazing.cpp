...
QString zibazing::dbaseName()
{
    ...
    QString dbName = workingDir().cleanPath("podcast_db.sqlite");
    qDebug() << "using db name: " << dbName;
    return dbName;
}

QDir zibazing::workingDir ()
{
    QDir l_podcastsDir (set.value("workingDir", QDir::homePath()+"//Podcasts").toString());
    set.setValue("workingDir", l_podcastsDir.absolutePath());
    return l_podcastsDir;
}
